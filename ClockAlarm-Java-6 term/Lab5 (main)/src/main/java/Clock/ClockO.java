...
//       t.interrupt();
    ...
    public void removealarm(int hour_, int minute_, int second_) {
        int[] time = new int[3];
        time[0] = hour_;
        time[1] = minute_;
        time[2] = second_;
        Print.print("I am searching for: " + hour_ + minute_ + second_);
        for (AlarmO al: this.list) {
            Print.print("I found: "+ al.GetTime()[0] + al.GetTime()[1] + al.GetTime()[2]);
//            if (al.GetTime().equals(time)) {  ТАК НЕ РАБОТАЕТ ТУПОЙ ДЖАВА???
            if (al.GetTime()[0] == time[0] && al.GetTime()[1] == time[1] && al.GetTime()[2] == time[2]) {
                Print.print("Found alarm - remove");
                al.setWork(false);
            }
        }
    }
